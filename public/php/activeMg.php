...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES["active_img"]) && $_FILES["active_img"]["error"] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES["active_img"]["tmp_name"], "{$targetDirectory}{$targetFile}")) {
            ...
            $update_sql = "UPDATE active SET active_category = :active_category, active_content = :active_content, active_title = :active_title, active_img = :active_img, active_state = :active_state, active_date = :active_date, active_star = :active_star, active_end = :active_end WHERE active_id = :active_id";
        ...
        $update_sql = "UPDATE active SET active_category = :active_category, active_content = :active_content, active_title = :active_title, active_state = :active_state, active_date = :active_date, active_star = :active_star, active_end = :active_end WHERE active_id = :active_id";
        $update_stmt = $pdo->prepare($update_sql);
        $update_stmt->bindValue(":active_id", $active_id, PDO::PARAM_INT);
        $update_stmt->bindValue(":active_title", $active_title);
        $update_stmt->bindValue(":active_content", $active_content);
